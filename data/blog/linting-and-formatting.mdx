...
And now we're on to the big-daddy. [ESLint](https://eslint.org/) is the most powerful static analysis tool we have at our disposal for JavaScript and TypeScript code, thanks a lot to its massive suite of native and third-party rules and plugins.
...
Unlike the other tools I mention in this article, ESLint is a little more complex and not practical to explain in full here. If you're completely new to it, I recommend checking out [their page on core concepts](https://eslint.org/docs/latest/use/core-concepts/), then returning to this article.
...
Both core ESLint and most plugins you will use have a large number of rules and it can be quite overwhelming to decide which are enabled. Thankfully, most of the time they also come with a "recommended" set of rules. These are rules that are desirable for the vast majority of projects, and can be turned on with a single line of configuration.
