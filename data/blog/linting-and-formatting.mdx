...
_All tools and configurations described in this article are shown in this [demo repository](https://github.com/fnannestad/formatting-linting-demo)._

---

If there was a Nitpickers Anonymous support group, I'd be member number one.

Poorly named variables, line breaks in the wrong place, typos, exports that are never imported, ternary statements nested within ternary statements, functions with too many arguments - these are the things that keep me up at night.
...
Due to the breadth of these rules it can also be a overwhelming to know where to start and what rules you should be turning on. I'll aim to demystify it a little and demonstrate that we can get 99% of the benefit of ESLint with just a handful of plugins, and no complex configuration.

## Concepts

Unlike the other tools covered in this article ESLint is a little more complex and not practical to explain in full here, however the three main concepts to understand are:

1. Rules - each of these validates that your code meets a certain condition, for example, whether your function has a return type specified. Different rules have different configuration options, and generally can be set to either "warn" or "error". Code breaking the rule will get a zigzag underline in your IDE indicating which rule was broken. Some of these are autofixing by running ESLint, but some require manual intervention.
2. Plugins - npm module that contains third party sets of rules and configurations
3. Sharable configurations - configurations shared via npm that enable rules build into ESLint or plugins
...
_As ESLint configuration is quite wordy, I won't provide complete examples here. If you are after exact syntax, check out the [ESLint config in the aforementioned demo repository](https://github.com/fnannestad/formatting-linting-demo/blob/main/eslint.config.ts) which contains all configuration talked through here. Please note that here I have used TypeScript for my config file. If you wish to do the same, you need to also add `jiti` to your dev-dependencies._

## Recommended configuration

Both core ESLint and most plugins you will use have a large number of rules and it can be quite overwhelming to decide which are enabled. Thankfully, most of the time they also come with a sharable configurations which are often "recommended" set of rules. These are rules that are desirable for the vast majority of projects, and can be turned on without needing to specify each rule individually.
...
Add the plugin and apply the `recommended` configuration.
...
This doesn't fall perfectly into the category of linting and formatting, but there are a few type-checking configuration options for the TypeScript compiler that are simply too powerful not to mention, both from a code cleanliness and reduced-risk-of-bugs perspective.
...
Special emphasis on `strictNullChecks`. If you don't like seeing crashes with a message `Uncaught TypeError: Cannot read properties of undefined`, this is the option for you.
...
Thankfully, there are a few strategies we can use to automate execution of our tooling for developers to make their lives easier and to prevent their code from reaching `master` if it makes it that far.
...
2. Our new status check is required to pass to merge a pull request

The next time someone raises a pull request that doesn't pass our checks, they will see a UI that looks like this:
...
While unfortunately there isn't yet a complete antidote to the spectre of "bad code", we've now applied tooling and settings that will help (and force) our team to write code as readable, consistent and bug free as possible.

---

# Footnote - Was this even worth it?

Me rattling off what you might find elsewhere online would result in saying that automated linting and formatting:

1. Reduces the risk of bugs
2. Improves the readability of our code
3. Aids onboarding new-starters (who no longer need to read a lengthy style guide and just let the tooling do it for them)
...
These discussions can be mighty fun once in a while, but they aren't fun if they happen every week. Once we've come to a conclusion on one of these age-old debates, we should set it in stone by enforcing it with a lint rule so that we never have to discuss it again.
